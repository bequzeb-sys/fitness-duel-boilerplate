...
export default function CoachPage({
  ...
  return (
    <div className="space-y-6 text-left">
      ...
      {/* Sub-tab switcher */}
      <div className="bg-bg-card border border-border-card p-1.5 rounded-2xl flex gap-1.5 overflow-x-auto max-w-full custom-scrollbar z-10 select-none">
        <Button
        ...
        >
          ...
          <span>{t("chatTab")}</span>
          <span className="text-[9px] px-1.5 py-0.2 bg-white/20 text-white font-mono font-bold rounded-full">{t("live")}</span>
        </Button>
      ...
      {activeSubTab === "coach-ia" && (
        ...
        >
          ...
          <div className="lg:col-span-7">
            <Card padding="lg" className="flex flex-col justify-between h-[630px] min-h-[630px] space-y-4">
              {/* Header */}
              <div className="flex items-center justify-between pb-3.5 border-b border-border-card/80">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400 font-medium">{t("assistant")}</p>
                  </div>
                </div>
                <span className="text-[9px] font-mono text-emerald-400 font-bold bg-emerald-500/10 px-2 py-0.5 rounded flex items-center gap-1">
              ...
              {/* Messages */}
              <div className="flex-1 overflow-y-auto max-h-[380px] lg:max-h-[440px] pr-1.5 space-y-4 custom-scrollbar">
            ...
            <Card padding="lg" className="flex flex-col justify-between h-[630px] min-h-[630px] space-y-4">
              <div className="space-y-4 flex-1 flex flex-col">
                <div className="pb-2 border-b border-border-card/80 flex justify-between items-center">
                  ...
                  <span className="text-[9px] font-bold text-slate-500 font-mono">{HELP_AXES.length} {t("categories")}</span>
                </div>
                  ...
                  {HELP_AXES.map((ax) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <h5 className="font-display font-bold text-[11px] mt-1 text-slate-200 leading-tight">{t(ax.name)}</h5>
                      </Button>
                    );
                  })}
                </div>
                ...
                <div className="p-3.5 bg-surface-raised border border-border-card/80 rounded-xl flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-[10px] font-display font-bold text-white uppercase tracking-wider">{t(activeAxis.name)}</p>
                    <p className="text-[11px] text-slate-400 leading-tight mt-0.5">{t(activeAxis.shortDesc)}</p>
                  </div>
                </div>
                    ...
                    >
                      <div className="flex justify-between items-start gap-1 w-full">
                        <div>
                          ...
                          <p className="text-[9px] text-slate-500 leading-tight mt-0.5 font-medium line-clamp-1">{t(prog.descKey)}</p>
                        </div>
                        <span className="text-[8px] font-mono text-slate-500 shrink-0">{prog.duration}</span>
                      </div>
                    </Button>
                  ))}
                </div>
                          ...
                          <Card key={plan.id} variant="elevated" padding="md" className="flex flex-col justify-between space-y-3 relative group">
                            ...
                            <div className="pt-2 border-t border-border-card/80 flex justify-between items-center gap-2">
                              ...
                              <div className="flex items-center gap-1.5">
                                <Button variant="secondary" size="sm" onClick={() => copyToClipboard(plan.content)} className="text-[9px]">
                                  {t("copy")}
                                </Button>
