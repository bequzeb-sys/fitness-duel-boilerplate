    ...
    <Card padding="lg" className="flex flex-col justify-between h-[390px] min-h-[390px]">
      <div className="flex justify-between items-center pb-2 border-b border-border-card/60">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-bold px-1.5 py-0.2 bg-brand-blue text-white rounded uppercase tracking-wider scale-90">
            {t("live")}
          </span>
        </div>
        <Button variant="secondary" size="sm" onClick={onOpenFull} className="gap-1.5">
          <span className="text-brand-cyan text-[10px]">{t("viewDetails")}</span>
        </Button>
      </div>
        ...
        {messages.map((m) => (
          ...
          >
            <div className="flex items-center gap-1.5 mb-1">
              {m.sender === "coach" && (
                <div className="h-4 w-4 rounded bg-brand-blue flex items-center justify-center text-[10px] text-white font-bold">
                  A
                </div>
              )}
